...
class MainPage extends Component {
    ...
    getPost=()=>{ //API
        ...
        fetch('https://enterjain-b6e10e8b56bc.herokuapp.com/post')
        ...
        uploadTask.on(
          ...
          function () {
            uploadTask.snapshot.ref.getDownloadURL().then(function (downloadURL) {
                ...
                fetch("https://enterjain-b6e10e8b56bc.herokuapp.com/post",requestOptions)
                .then(response => response.json())
                .then(data => {
                    console.log(data);
                    thisContext.getPost();
            ...
            <div>
                ...
                <div className="upload_text">{this.state.progressBar}</div>
                {
                    this.state.postArray.map((item,index)=>(
                        ...
                        <Post keys = {item.postId} id={item.postId} userName={item.userName} postImage={item.postPath} likes={item.likeCount} />
                    ))
                }
            </div>
         );
